...
const Home = ()=>{
    ...
    function GetCommentFromApi(movieId) {
        ...
        fetch(`http://localhost:8080/api/comment/get?movieId=${movieId}`, {method: "GET"}).
        then(response => response.json()).
        then(response => {
    ...
    function GetCountComment(movieId) {
        ...
        fetch(`http://localhost:8080/api/comment/count?movieId=${movieId}`, {method: "GET"}).
        then(response => response.json()).
        then(response => {
        ...
        <div className="p-4 container-all">
            ...
            <div className={'d-flex flex-column'}>
                    <div className={'d-flex flex-wrap w-100 p-1 py-2 container__content'}>
                        <Poster poster={data?.thumbUrl}></Poster>
                    ...
                    <div className={'w-100 my-4 p-3 container__content'}>
                        ...
                        <div className='d-flex flex-wrap'>
                            {data.episodes?.map((episode, index)=><Episode key={index} episode={episode}></Episode>)}                      
                        </div>  
                        <TitleCollapse value="Bình luận"></TitleCollapse>  
                        <Comment getListComment={GetCommentFromApi} movieId={data?.id} listComment={listComment} countComment={countComment}></Comment>
                    </div>
                </div>
            </div>
            <Footer></Footer>
